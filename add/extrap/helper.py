...
def plot_zooms(naul, basel, fn, s, locc, box):
    ...
    t = np.arange(epoch_stop-35)
    basetemp = smooth(basel[(-epoch_stop+10):], 28)
    nautemp = smooth(naul[(-epoch_stop+10):], 5)
    
    basetemp = basetemp[14:-(-epoch_stop+21)]
    nautemp = nautemp[10:-(-epoch_stop+25)]
    ...
    plt.scatter([1980], [basetemp[len(basetemp)-1].item()], c='C3')
    plt.scatter([1980], [nautemp[len(nautemp)-1].item()], c='C0')
